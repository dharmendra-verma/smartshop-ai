...
async def test_classify_recommendation():
    ...
    with patch.object(clf._agent, "run", new_callable=AsyncMock) as m:
        r = MagicMock()
        r.output.intent = IntentType.RECOMMENDATION
        r.output.confidence = 0.95
        r.output.max_price = 500.0
...
async def test_classify_price():
    ...
    with patch.object(clf._agent, "run", new_callable=AsyncMock) as m:
        r = MagicMock()
        r.output.intent = IntentType.PRICE
        r.output.confidence = 0.92
        r.output.product_name = "Galaxy S24"
...
async def test_classify_policy():
    ...
    with patch.object(clf._agent, "run", new_callable=AsyncMock) as m:
        r = MagicMock()
        r.output.intent = IntentType.POLICY
        r.output.confidence = 0.88
        r.output.product_name = r.output.category = r.output.max_price = (
            r.output.min_price
        ) = None
...
async def test_classify_review():
    ...
    with patch.object(clf._agent, "run", new_callable=AsyncMock) as m:
        r = MagicMock()
        r.output.intent = IntentType.REVIEW
        r.output.confidence = 0.90
        r.output.product_name = "Sony WH-1000XM5"
        r.output.category = r.output.max_price = r.output.min_price = None
        r.output.reasoning = "review"
        m.return_value = r
        result = await clf.classify("What do customers say about Sony WH-1000XM5?")
    assert (
        result.intent == IntentType.REVIEW and result.product_name == "Sony WH-1000XM5"
    )
...
async def test_classify_extracts_price_range():
    ...
    with patch.object(clf._agent, "run", new_callable=AsyncMock) as m:
        r = MagicMock()
        r.output.intent = IntentType.RECOMMENDATION
        r.output.confidence = 0.93
        r.output.category = "laptops"
        r.output.max_price = 800.0
        r.output.min_price = 500.0
        r.output.product_name = None
        r.output.reasoning = "rec"
        m.return_value = r
